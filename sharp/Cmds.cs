...
public abstract class Cmd
{
    ...
    public static IEnumerator<object> Download(string url, long length, Stream copy)
    {
        ...
        do
        {
            ...
            if (length > 0)
                yield return StatusSilent(
                    $"Downloading: {(int)Math.Floor(100D * Math.Min(1D, pos / (double)length))}% @ {speed} KiB/s",
                    (float)(pos / progressScale / (double)progressSize), "download", true);
            else
                yield return StatusSilent($"Downloading: {(int)Math.Floor(pos / 1000D)}KiB @ {speed} KiB/s", false,
                    "download", true);
        } while (read > 0);
    ...
    public static IEnumerator Unpack(ZipArchive zip, string root, string prefix = "")
    {
        var count = string.IsNullOrEmpty(prefix)
            ? zip.Entries.Count
            : zip.Entries.Count(entry => entry.FullName.StartsWith(prefix));
